...
const App = (props)  =>{
  ...
  const subCrop=()=>{
    ...
    async function croping(){
      ...
      const division      =   croppedImage.length / tamano_partes_img
      let imagen_cortada  =   []
      const total         =   Math.ceil(division)
      token_temporal      =   Math.random()
      let quede_en        =   0
      //return console.log(croppedImage);
      for (var i = 0; i < total; i++) {
        let part  = croppedImage.substring(quede_en,tamano_partes_img * (i+1))
        imagen_cortada.push(part)
        quede_en    =   tamano_partes_img * (i+1);
      }

        //console.log(imagen_cortada);
        partes_imagen=imagen_cortada
        uploadPart_()
      ...
      //     inputs_.image_parts       =   imagen_cortada
      //     setInputs(inputs_)
