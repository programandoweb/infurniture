...
if (!window.indexedDB) {
  ...
  InitDb              =   async function InitDb(data) {
    ...
    request.onupgradeneeded = function(event) {
      ...
      Object.entries(data).map((v,k)=>{
        ...
        return Object.entries(data).map((v2,k2)=>{
          return objectStore.createIndex(v2[1].name, v2[1].name, { unique: false });
        })
      })
    };
  ...
  Add_evaluations_by_child = async function Add_evaluations_by_child(data) {
    ...
    request.onupgradeneeded = function(event) {
      ...
      objectStore.createIndex("evaluacion", "evaluacion", { unique: false });
      ...
      Object.entries(data.data.evaluations_by_child).map((v2,k2)=>{
        let alumno_id = v2[0];
        return Object.entries(v2[1]).map((v3,k3)=>{
          let info                =   v3[1];
      ...
      objectStore     =     db.createObjectStore("ListaAlumnosMaterias", {keyPath:'id',autoIncrement: true} );
      objectStore.createIndex("alumno_id", "alumno_id", { unique: false });
      Object.entries(data.data.ListaAlumnosMaterias).map((v2,k2)=>{
        var request_i           =   objectStore.add({alumno_id:v2[0],materias:v2[1]});
            request_i.onsuccess = function(event) {};
            return request_i
      })


      objectStore     =     db.createObjectStore("ListaProfesores", {keyPath:'id',autoIncrement: true} );
      objectStore.createIndex("materias", "materias", { unique: false });
      objectStore.createIndex("profesor_id", "profesor_id", { unique: false });
      objectStore.createIndex("nombres", "nombres", { unique: false });

      data.data.ListaProfesores.map((v2,k2)=>{
        var request_i           =   objectStore.add(v2);
            request_i.onsuccess = function(event) {};
            return request_i
      })
  ...
  GetProfesores     =   async function GetEvaluaciones(){
    return new Promise(function(resolve, reject){
        let request       =   indexedDB.open(db_name,1);
        request.onerror   =   function(event) {
          //console.log(event);
        ...
        request.onsuccess =   function(event) {
          db              =   event.target.result;
          var objectStore =   db.transaction("ListaProfesores").objectStore("ListaProfesores");
          objectStore.getAll().onsuccess = function(event) {
            resolve(event.target.result);
          }
        }
  ...
  GetMaterias     =   async function GetEvaluaciones(){
    return new Promise(function(resolve, reject){
        let request       =   indexedDB.open(db_name,1);
        request.onerror   =   function(event) {
          //console.log(event);
        ...
        request.onsuccess =   function(event) {
          db              =   event.target.result;
          var objectStore =   db.transaction("ListaAlumnosMaterias").objectStore("ListaAlumnosMaterias");
          objectStore.getAll().onsuccess = function(event) {
            resolve(event.target.result);
          }
        }
